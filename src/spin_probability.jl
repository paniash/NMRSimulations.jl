...
function prob_down(func, u0, tspan, p)
...
plt.title("Probability of spin down")
plt.plot(prob_down(bloch!, u0, tspan, p)[1], prob_down(bloch!, u0, tspan, p)[2])
plt.xlabel("t (ns)")
plt.ylabel("Probability of spin down")
...
for i = 1:N
    ω = omega_array[i]
    p = [ω, ω0, ω1]
    p_b = prob_down(bloch!, u0, tspan, p)[2]
...
for i = 1:N
    ω = omega_array[i]
    param = [ω, ω0, ω1]
    p_b = prob_down(bloch!, u0, tspan, param)[2]
...
function eval_power_spectrum(u0, tspan, param)
    p_b = prob_down(bloch!, u0, tspan, param)[2]
    t_array = prob_down(bloch!, u0, tspan, param)[1]
    fft_pb = fft(p_b)
